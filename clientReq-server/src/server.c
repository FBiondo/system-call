...
struct Entry *shm_entry; //shared memory
int semid; //semaphore
...
void sendResponse(struct Request *request) {
    ...
    struct Entry myEntry; //struttura del keymanager
...
void addEntry(struct Entry* shm_entry, struct Entry myEntry, int* n) {
    strcpy(shm[*n].userID, myEntry.userID);
    shm_entry[*n].key = myEntry.key;
    shm_entry[*n].timeStart = myEntry.timeStart;
}
void swap(struct Entry* shm_entry, int x, int y) {
    struct entry_t tmp = *(shm_entry + x);
    *(shm + x) = *(shm + y);
    *(shm + y) = tmp;
}

void delEntry(struct Entry* shm_entry, int* n) {
